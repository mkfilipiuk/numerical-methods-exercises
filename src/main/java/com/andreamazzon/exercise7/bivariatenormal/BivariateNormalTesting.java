...
public class BivariateNormalTesting {
        ...
        switch (method) {// name of the method
            case BIVARIATENORMAL:
                ...
                for (int i = 0; i < numberOfComputations; i++) {
                    ...
                    for (int j = 0; j < numberOfDrawings; j++) {
                        var sampled = normalTestSampler.generateBivariateNormal();
                        if (sampled[0] < mu && sampled[1] < mu) {
            ...
            case ACCEPTANCEREJECTION:
                ...
                for (int i = 0; i < numberOfComputations; i++) {
                    ...
                    long lStartTime = System.currentTimeMillis();// time when the computations starts
                    for (int j = 0; j < numberOfDrawings; j++) {
                        var sampled = normalTestSampler.generateBivariateNormalAR();
                        if (sampled[0] < mu && sampled[1] < mu) {
                            numberOfTimesBothSmallerThanMu++;
                        }
                    }
            ...
            case BOXMULLER:
                ...
                for (int i = 0; i < numberOfComputations; i++) {
                    ...
                    long lStartTime = System.currentTimeMillis();// time when the computations starts
                    for (int j = 0; j < numberOfDrawings; j++) {
                        var sampled = normalTestSampler.generateBoxMuller();
                        if (sampled[0] < mu && sampled[1] < mu) {
                            numberOfTimesBothSmallerThanMu++;
                        }
                    }
            ...
            case ARBOXMULLER:
                ...
                for (int i = 0; i < numberOfComputations; i++) {
                    ...
                    for (int j = 0; j < numberOfDrawings; j++) {
                         ...
                         */
                        var sampled = normalTestSampler.generateARBoxMuller();
                        if (sampled[0] < mu && sampled[1] < mu) {
                            numberOfTimesBothSmallerThanMu++;
                        }
                    }
    ...
    public static void main(String[] args) {
        ...
        for (var method : GenerationMethods.values()) {
            testTheMethods(normalTestSampler, method, numberOfDrawings, numberOfComputations);
        }
    }
}
